...
Note that the procedure _quotient_of_preimage(_preimage_of_partition(l)) is equal to the procedure _epi_factorize_partition(l)
...
def _quotient_of_preimage(preimage):
  ...
  for i in range(len(preimage)):
    for j in range(len(preimage[i])):
      the_quotient.append("?")
  #Now that the partition has the right number of allocated spaces in the memory,
