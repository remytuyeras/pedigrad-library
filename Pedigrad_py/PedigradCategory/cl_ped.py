...
- This list of strings is then relabeled with respect the indices of the lists contained in the equivalence classes that are meant to be given to the constructor of LocalAnalysis;
...
The method .isolate takes an integer and a list of characters and 
- if the integer is equal to the global variable NEW (i.e. equal to 1), then the procedure returns a new Pedigrad item;
- otherwise, the procedure changes the ambient pedigrad directly.

If the input integer is equal to NEW, then the procedure returns a pedigrad that has the same attributes as the ambient pedigrad, except for its object .local whose characters that belong to the input list are given new and distinguished labels so that they give rise to singleton parts in the partitioning given by the images of the pedigrad.

If the input integer is not equal to NEW, then the procedure replaces those characters of each internal list of self.local that belong to the input list with new and distinguished characters so that these give rise to singleton parts in the partitioning given by the images of the pedigrad.

The method .agree takes a list of SegmentObject items and a list of non-negative integers (i.e. a partition) and returns all those segments contained in the first input for which there is a morphism of partitions from the second input to the outputs of the procedure self.partition(-,SEGM_MODE) at these segments.
...
class Pedigrad(LocalAnalysis): 
  ...
  def partition(self,*args):
    ...
    #in the second case (see cl_la.py).
      ...
      for i in range(len(self.base)):
        if self.homset(the_segment,self.base[i]):
          the_image = product_of_partitions(the_image,self.local[i])
      #The resulting product of partitions is returned
      return the_image

  #The variable 'update_mode' is an integer (which may be given by the 
  #global variable NEW = 1) and exceptions is a list.
  def isolate(self,update_mode,exceptions):
    #If the input 'update_mode' is equal to NEW, then the procedure
    #creates a new pedigrad, otherwise it only changes self.local.
    if update_mode == NEW:
      #A new Pedigrad item is allocated in the memory. Since the name of the
      #file passed in its first argument is empty, the objects .local and 
      #.taxa of the variable 'new_pedigrad' are empty. Also, because 
      #the name of the preorder structure is empty, the object .mask is 
      #set to False and the preorder is encoded by an empty list 
      #(this is rectified below).
      new_pedigrad = \
      Pedigrad('',0,SEGM_MODE,self.base,self.equiv,'',self.domain)
    ...
    #If the variable 'update_mode' is equal to NEW, then the following lines
    #fill the i-th position of the object .local of 'new_pedigrad' with 
    #the partition resulting from changing all the labels of self.local[i]
    #contained in exceptions with distinguished string symbols 
    #e.g. the string ">n", where n is a number. 
    #If "update_mode" is not equal to NEW, then it only replaces all those
    #characters self.local[i] contained in exceptions with a distinguished
    #string symbols of the form '>'+number.
    for i in range(len(self.local)):
      #The variable 'count' is meant to contain the number n that is used 
      #to make the new labels of those characters whose labels are 
      #in exceptions.
      count = 0
      #If 'self.local[i]' contains an element of 'exceptions' at position j,
      #then the new label for this position is '>'+str(count). 
      #The fact that one uses the symbol '>' is due to the fact that the
      #symbol  '>' is not supposed to be part of the type of the characters
      #used in the sequence alignment. If exceptions is empty, then the list
      #self.local is copied in new_pedigrad.local.
      new_column = list()
      for j in range(len(self.local[i])):
        if self.local[i][j] in exceptions:
          if update_mode == NEW:
            new_column.append('>'+str(count))
          else:
            self.local[i][j] = '>'+str(count)
          #The next character to be relabeled should receive a label that 
          #has not already been used in self.local[i]. In this respect,
          #the integer in 'count' is incremented.
          count = count + 1
        else:
          if update_mode == NEW:
            new_column.append(self.local[i][j])
      #Once the labels of the columns of the matrix .local have been checked
      #and potentially modified, they are added to the object .local of the
      #new pedigrad if 'update_mode" is equal to NEW.
      if update_mode == NEW:
        new_pedigrad.local.append(new_column)
    #The new pedigrad is returned with an updated .local.
    if update_mode == NEW:
      return new_pedigrad

  #The variable 'ground' is meant to be a list of SegmentObject items 
  #while the variable 'partition' is meant to be a list of 
  #non-negative integers.
  def agree(self,ground,partition):
    ...
    #will record all the segments whose images via the pedigrad (self) receive
    #a morphism of partitions from the list 'partition'.
    agreeing_segments = list()
    #The following loop collects the segments belonging to the list 'ground'
    #for which there is a morphism of partitions from 'partition' to the 
    #images of the pedigrad valuated at these segments.
    for i in range(len(ground)):
      if homset(partition,self.partition(ground[i],SEGM_MODE)):
        #The segments for which the morphism exist are 
        #stored in agreeing_segments
