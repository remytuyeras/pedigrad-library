...
The consructor .__init__ takes the name of a file and an integer (as in the case of the function _reads_alignment_file) and stores 
- the first output of _reads_alignment_file in the object .taxa;
- the transpose of the second output of _reads_alignment_file (when seen as a matrix), up to removal of those rows that contain the same character, in .local;
- the indices of the lists contained in .local as indexed the in the second output of _reads_alignment_file in the object .loci (see below).
...
'''
from raf import _reads_alignment_file
...
class Pedigrad: 
  ...
  def __init__(self,name_of_file,reading_mode):
    ...
    if name_of_file == "":
      ...
      self.loci = list()
      self.taxa = list()
    #Otherwise, the procedure _reads_alignment_file reads the file
    #and takes care of the possible exceptions.
    else:
      output = _reads_alignment_file(name_of_file,reading_mode)
      self.taxa = output[0]
      alignment = output[1]
      if len(self.taxa) != 0:
        ...
        for i in range(len(self.taxa)-1):
          if len(alignment[0]) != len(alignment[i+1]):
            #flag is set to 1 if the rows of the matrix 
  ...
  def partition(self,segment):
    ...
    #the list stored in self.local
        ...
        the_image = _epi_factorizes_partition(self.local[segment[0]]) 
